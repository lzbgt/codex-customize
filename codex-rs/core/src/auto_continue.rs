...
mod tests {
    ...
    fn followup_prompt_sets_high_throughput_targets() {
        assert!(
            AUTO_CONTINUE_FOLLOWUP_PROMPT.contains("12-18"),
            "followup prompt should enforce higher batch targets"
        );
        assert!(
            AUTO_CONTINUE_FOLLOWUP_PROMPT.contains("12+"),
...
- Make each turn dense: complete a cluster of related tasks (prefer 12-18) including implementation + tests + docs + verification; avoid one-off micro fixes.\n\
- Minimum bar: deliver 12+ meaningful tasks per turn; if scope is thin, expand into adjacent tests, docs, tooling, and config hardening.\n\
...
3) Else: pick a batch of high-leverage tasks (typically 12-18) that compound and reduce future maintenance.\n\
...
- Aim for substantial progress per turn; batch 12-18 related tasks when possible and avoid tiny tweaks.\n\
- Minimum bar: deliver 12+ meaningful tasks per turn (code + tests + docs or adjacent feature work).\n\
...
- Deliver multiple substantial changes per turn (target 12-18 related tasks); do not stop after a single small change.\n\
